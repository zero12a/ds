...
firepad.FirebaseAdapter = (function (global) {
  ...
  function FirebaseAdapter (ref, userId, userColor, userName) {
    alog("FirebaseAdapter(ref, userId, userColor, userName)..................................start");
    alog("  userId = " + userId);
    alog("  userColor = " + userColor);
  ...
  FirebaseAdapter.prototype.initializeUserData_ = function() {
    alog("initializeUserData_().............................start");
  ...
  FirebaseAdapter.prototype.monitorCursors_ = function() {
    ...
    function childChanged(childSnap) {
      ...
      alog("  userId = " + userId);
      alog("  color = " +  userData.color);
      alog("  name = " +  userData.name);
      alog("  cursor = " +  JSON.stringify(userData.cursor,null,2));


      if(userData.cursor === undefined && 1==2){
        //사라지게 하지마
      }else{
...
firepad.EditorClient = (function () {
  ...
  OtherClient.prototype.updateCursor = function (cursor) {
    alog("updateCursor()...................................start");
    alog("  this.id = " + this.id);
    alog("  this.color = " + this.color);
    alog("  this.name = " + this.name);
  ...
  function EditorClient (serverAdapter, editorAdapter) {
    ...
    this.serverAdapter.registerCallbacks({
      ...
      cursor: function (clientId, cursor, color, name) {
        alog("registerCallbacks() cursor.............................start");
        alog("  clientId = " + clientId);
        alog("  cursor = " + cursor);
        alog("  color = " + color);
        alog("  name = " + name);
...
firepad.RichTextCodeMirrorAdapter = (function () {
  ...
  RichTextCodeMirrorAdapter.prototype.setOtherCursor = function (cursor, color, clientId, clientNm) {
    alog("RichTextCodeMirrorAdapter.setOtherCursor().................................start");
    alog("  color = " + color);
    alog("  clientId = " + clientId);
    alog("  clientNm = " + clientNm);

    var cursorPos = this.cm.posFromIndex(cursor.position);
    if (typeof color !== 'string' || !color.match(/(^#[a-fA-F0-9]{3,6}$)|([a-zA-Z]+)/)) { //컬러표 정규식 검사 변경함........
      return;
    }
    ...
    if (cursor.position === cursor.selectionEnd) {
      ...
      //cursorEl.style.width = "200px";
      //cursorEl.style.height = "200px";
      //cursorEl.style.zIndex = 999;
      //cursorEl.style.backgroundColor = "red";
      //alog(cursorEl.style);
      ...
      //spanName.id = "tooltip";
      //cursorEl.appendChild( spanName );
      //var codeMirrorCode = document.querySelector(".CodeMirror-lines");
      
      //var firepadContainer = document.getElementById('firepad-container');
      //var coordsF = firepadContainer.getBoundingClientRect();

      //message.style.left = coordsF.left + "px";
      //message.style.top = coordsF.bottom + "px";

      //alog(coordsF);


      //alog(codeMirrorCode);
      cursorEl.append( spanName );

      //document.body.appendChild( spanName );
      spanName.style.position = "fixed"; //absolute, fixed
      //spanName.style.position = "static"; //absolute, fixed
      spanName.id = clientId;
      spanName.innerHTML = clientNm;
      //spanName.appand( document.createTextNode("Teacher node") );
      spanName.style.backgroundColor = color;
      //alert(cursorCoords.left + "px");
      spanName.style.left = parseInt(cursorCoords.left - 30) + "px";
      //spanName.style.opacity = ".99";
      //var loc = "";
      ...
      if(cursorPos.line == 0){
        //alert("라인 0");
        alog("cursorEl.style.height = " + cursorEl.style.height);
        spanName.style.zIndex = 4;
        spanName.style.top = (cursorCoords.bottom - cursorCoords.top) + "px"; //이거로 하면 투명으로 아래줄과 겹침
        //spanName.style.marginTop = cursorEl.style.height;//이거로 하면 투명으로 아래줄과 겹침
      }else{
        */
        alog("cursorCoords.........................................val");
        alog(cursorCoords)
        //spanName.style.zIndex = 4;
        //spanName.style.top = "-" + (cursorCoords.bottom - cursorCoords.top + 5) + "px"; //padding 위 아래 숫자도 더해주기
        spanName.style.top = (cursorCoords.top - parseInt(cursorEl.style.height) - 5 ) + "px"; //padding 위 아래 숫자도 더해주기
        spanName.style.left = cursorCoords.left + "px"; //padding 위 아래 숫자도 더해주기
        //spanName.style.marginBottom = (parseInt(cursorEl.style.height)+5) + "px";
      //}
      //spanName.style.top = "-20px";
      //spanName.style.left = "-20px";

      spanName.style.marginLeft = "-2px";
      //spanName.style.zIndex = 1000;
      spanName.style.padding = "3px 4px 2px 6px";
      //spanName.style.opacity = "0.7";//투명도
      spanName.style.height = cursorEl.style.height;
      spanName.style.lineHeight = cursorEl.style.height;
      //spanName.style.width = '100px';
      spanName.style.borderRadius = "5px"; 


      //alog("cursorPos = " + JSON.stringify(cursorPos));
      //alog("\t cursorCoords.bottom = " + cursorCoords.bottom);
      //alog("\t cursorCoords.top = " + cursorCoords.top);


      //this.cm.setBookmark(cursorPos,{ widget: spanName, insertLeft: true });

      //Popper.createPopper(cursorEl, spanName, {
      //  placement: loc,
      //});
      ...
      var obj = this.cm.markText(fromPos, toPos, {
        className: selectionClassName
      });
      alog("Marker.find = " + JSON.stringify(obj.find()));
      //html오브젝트가 생기는게 아니라, class에 "selection-컬러명"이 추가되기 때문에, 하위 오브젝트 추가가 안됨.
...
firepad.Firepad = (function(global) {
  ...
  function Firepad(ref, place, options) {
    alog("Firepad(ref, place, options)....................................start")
    ...
    this.firebaseAdapter_.on('cursor', function() {
      alog("this.firebaseAdapter_.on('cursor').................................start");
      alog(arguments)
