	...
	"G1", 
		{
			...
			,"SEQYN": "N"
			,"COLS": [
				{ "COLID": "TEAMCD", "COLNM" : "TEAMCD", "OBJTYPE" : "INPUTBOX" }
...
,				{ "COLID": "INTRO_PGMID", "COLNM" : "INTRO_PGMID", "OBJTYPE" : "TEXT" }
...
//글로벌 변수 선언
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_SEARCHALL = "rdteammngController?CTLGRP=G1&CTLFNC=SEARCHALL";
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_SAVE = "rdteammngController?CTLGRP=G1&CTLFNC=SAVE";
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_RESET = "rdteammngController?CTLGRP=G1&CTLFNC=RESET";
//조회조건 변수 선언	
var obj_G1_TEAMCD; // TEAMCD 변수선언
var obj_G1_TEAMNM; // TEAMNM 변수선언
//컨트롤러 경로
var url_G2_SEARCH = "rdteammngController?CTLGRP=G2&CTLFNC=SEARCH";
//컨트롤러 경로
var url_G2_SAVE = "rdteammngController?CTLGRP=G2&CTLFNC=SAVE";
//컨트롤러 경로
var url_G2_ROWDELETE = "rdteammngController?CTLGRP=G2&CTLFNC=ROWDELETE";
//컨트롤러 경로
var url_G2_ROWADD = "rdteammngController?CTLGRP=G2&CTLFNC=ROWADD";
//컨트롤러 경로
var url_G2_RELOAD = "rdteammngController?CTLGRP=G2&CTLFNC=RELOAD";
//컨트롤러 경로
var url_G2_HIDDENCOL = "rdteammngController?CTLGRP=G2&CTLFNC=HIDDENCOL";
//컨트롤러 경로
var url_G2_EXCEL = "rdteammngController?CTLGRP=G2&CTLFNC=EXCEL";
//컨트롤러 경로
var url_G2_CHKSAVE = "rdteammngController?CTLGRP=G2&CTLFNC=CHKSAVE";
//그리드 객체
var wixdtG2,isToggleHiddenColG2,lastinputG2,lastinputG2json,lastrowidG2;
var lastselectG2json;
var G2_REQUEST_ON = false;
//컨트롤러 경로
var url_G3_SEARCH = "rdteammngController?CTLGRP=G3&CTLFNC=SEARCH";
//컨트롤러 경로
var url_G3_RELOAD = "rdteammngController?CTLGRP=G3&CTLFNC=RELOAD";
//컨트롤러 경로
var url_G3_CHKSAVE = "rdteammngController?CTLGRP=G3&CTLFNC=CHKSAVE";
//그리드 객체
...
function G2_SAVE(token){
	...
	$.ajax({
		...
		error: function(error){
			...
			if(error.status == 200){
				msgError("[팀 목록] Ajax http error ( Response status is " + error.status + ", Not json format, Check console log )", 3);
				alog("	responseText" + error.responseText);//not json format
			}else if(error.status == 500){
				msgError("[팀 목록] Ajax http error ( Response status is " + error.status + ", Server error occer, Check console log )", 3);
				alog("	responseText" + error.responseText); //Server don't response
			}else if(error.status == 0){
				msgError("[팀 목록] Ajax http error ( Response status is " + error.status + ", Server don't resonse, Check console log )", 3);
				alog("	responseJSON = " + error.responseJSON); //Server don't response
			}else{
				msgError("[팀 목록] Ajax http error ( Response status is " + error.status + ", Server unknown resonse, Check console log )", 3);
...
function G2_SEARCH(tinput,token){
	...
	$.ajax({
		...
		success: function(data){
			...
			msgNotice("[팀 목록] 조회 성공했습니다. ("+row_cnt+"건)",1);

			}else{
				msgError("[팀 목록] 서버 조회중 에러가 발생했습니다.RTN_CD : " + data.RTN_CD + "ERR_CD : " + data.ERR_CD + "RTN_MSG :" + data.RTN_MSG,3);
			}
		},
		error: function(error){
...
function G3_SEARCH(tinput,token){
	...
	$.ajax({
		...
		success: function(data){
			...
			msgNotice("[미 등록팀] 조회 성공했습니다. ("+row_cnt+"건)",1);

			}else{
				msgError("[미 등록팀] 서버 조회중 에러가 발생했습니다.RTN_CD : " + data.RTN_CD + "ERR_CD : " + data.ERR_CD + "RTN_MSG :" + data.RTN_MSG,3);
			}
		},
		error: function(error){
			...
			alog("Response ajax error occer.");
			if(error.status == 200){
				msgError("[미 등록팀] Ajax http error ( Response status is " + error.status + ", Not json format, Check console log )", 3);
				alog("	responseText" + error.responseText);//not json format
			}else if(error.status == 500){
				msgError("[미 등록팀] Ajax http error ( Response status is " + error.status + ", Server error occer, Check console log )", 3);
				alog("	responseText" + error.responseText); //Server don't response
			}else if(error.status == 0){
				msgError("[미 등록팀] Ajax http error ( Response status is " + error.status + ", Server don't resonse, Check console log )", 3);
				alog("	responseJSON = " + error.responseJSON); //Server don't response
			}else{
				msgError("[미 등록팀] Ajax http error ( Response status is " + error.status + ", Server unknown resonse, Check console log )", 3);
				alog(error); //Server don't response
			}
