...
//글로벌 변수 선언
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_SEARCHALL = "pgmsearchController?CTLGRP=G1&CTLFNC=SEARCHALL";
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_SAVE = "pgmsearchController?CTLGRP=G1&CTLFNC=SAVE";
//버틀 그룹쪽에서 컨틀롤러 호출
var url_G1_RESET = "pgmsearchController?CTLGRP=G1&CTLFNC=RESET";
// 변수 선언	
var obj_G1_PGMID; // 프로그램ID 변수선언
var obj_G1_PGMNM; // 프로그램이름 변수선언
//그리드 변수 초기화	
//컨트롤러 경로
var url_G2_SEARCH = "pgmsearchController?CTLGRP=G2&CTLFNC=SEARCH";
//컨트롤러 경로
var url_G2_SAVE = "pgmsearchController?CTLGRP=G2&CTLFNC=SAVE";
//컨트롤러 경로
var url_G2_ROWDELETE = "pgmsearchController?CTLGRP=G2&CTLFNC=ROWDELETE";
//컨트롤러 경로
var url_G2_ROWBULKADD = "pgmsearchController?CTLGRP=G2&CTLFNC=ROWBULKADD";
//컨트롤러 경로
var url_G2_ROWADD = "pgmsearchController?CTLGRP=G2&CTLFNC=ROWADD";
//컨트롤러 경로
var url_G2_RELOAD = "pgmsearchController?CTLGRP=G2&CTLFNC=RELOAD";
//컨트롤러 경로
var url_G2_HIDDENCOL = "pgmsearchController?CTLGRP=G2&CTLFNC=HIDDENCOL";
//컨트롤러 경로
var url_G2_EXCEL = "pgmsearchController?CTLGRP=G2&CTLFNC=EXCEL";
//그리드 객체
var mygridG2,isToggleHiddenColG2,lastinputG2,lastinputG2json,lastrowidG2;
var lastselectG2json;//그리드 변수 초기화	
//컨트롤러 경로
var url_G3_SEARCH = "pgmsearchController?CTLGRP=G3&CTLFNC=SEARCH";
//컨트롤러 경로
var url_G3_SAVE = "pgmsearchController?CTLGRP=G3&CTLFNC=SAVE";
//컨트롤러 경로
var url_G3_ROWDELETE = "pgmsearchController?CTLGRP=G3&CTLFNC=ROWDELETE";
//컨트롤러 경로
var url_G3_ROWBULKADD = "pgmsearchController?CTLGRP=G3&CTLFNC=ROWBULKADD";
//컨트롤러 경로
var url_G3_ROWADD = "pgmsearchController?CTLGRP=G3&CTLFNC=ROWADD";
//컨트롤러 경로
var url_G3_RELOAD = "pgmsearchController?CTLGRP=G3&CTLFNC=RELOAD";
//컨트롤러 경로
var url_G3_HIDDENCOL = "pgmsearchController?CTLGRP=G3&CTLFNC=HIDDENCOL";
//컨트롤러 경로
var url_G3_EXCEL = "pgmsearchController?CTLGRP=G3&CTLFNC=EXCEL";
//그리드 객체
...
function G2_INIT(){
  alog("G2_INIT()-------------------------start");
	...
	//헤더초기화
	mygridG2.setHeader("PJTSEQ,프로젝트ID,프로젝트명");
	mygridG2.setColumnIds("PJTSEQ,PJTID,PJTNM");
	mygridG2.setInitWidths("40,60,60");
	mygridG2.setColTypes("ed,ed,ed");
	//가로 정렬	
	mygridG2.setColAlign("left,left,left");
	mygridG2.setColSorting("int,str,str");	//렌더링	
	mygridG2.enableSmartRendering(true);
	mygridG2.enableMultiselect(true);
	//mygridG2.setColValidators("G2_PJTSEQ,G2_PJTID,G2_PJTNM");
...
function G3_INIT(){
  alog("G3_INIT()-------------------------start");
	...
	//헤더초기화
	mygridG3.setHeader("프로그램ID,프로그램이름,ADDDT,MODDT");
	mygridG3.setColumnIds("PGMID,PGMNM,ADDDT,MODDT");
	mygridG3.setInitWidths("100,100,60,60");
	mygridG3.setColTypes("ed,ed,ro,ro");
	//가로 정렬	
	mygridG3.setColAlign("left,left,left,left");
	mygridG3.setColSorting("str,str,str,str");	//렌더링	
	mygridG3.enableSmartRendering(true);
	mygridG3.enableMultiselect(true);
	//mygridG3.setColValidators("G3_PGMID,G3_PGMNM,G3_ADDDT,G3_MODDT");
	...
	//onCheck
		...
		mygridG3.attachEvent("onRowSelect",function(rowID,celInd){
			...
			popG3json = jQuery.parseJSON('{ "__NAME":"lastinputG3json"' +
				', "CD" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("PGMID")).getValue()) + '"' +
				', "NM" : "' + q(mygridG3.cells(rowID,mygridG3.getColIndexById("PGMNM")).getValue()) + '"' +
			'}');
	...
	var conAllData = "";
        ...
        $.ajax({
            ...
            error: function(error){
                ...
                alog("[G2] Ajax http 500 error ( " + data.RTN_MSG + " )");
            }
        });
        alog("G2_SEARCH()------------end");
    }
...
function G2_EXCEL(){	
	...
	$("#DATA_HEADERS").val("PJTSEQ,PJTID,PJTNM");
	$("#DATA_WIDTHS").val("40,60,60");
...
function G3_EXCEL(){	
	...
	$("#DATA_HEADERS").val("PGMID,PGMNM,ADDDT,MODDT");
	$("#DATA_WIDTHS").val("100,100,60,60");
	$("#DATA_ROWS").val(myXmlString);
	myForm.submit();
}
	...
	var conAllData = "";
        ...
        $.ajax({
            ...
            error: function(error){
                ...
                alog("[G3] Ajax http 500 error ( " + data.RTN_MSG + " )");
            }
        });
        alog("G3_SEARCH()------------end");
    }
...
//그리드 행추가 : G3
	function G3_ROWADD(){
		if( !(lastinputG3)|| lastinputG3.get("G3-PJTSEQ") == ""){
			msgError("조회 후에 행추가 가능합니다. 또는 상속값이 없습니다.",3);
		}else{
			var tCols = ["","","",""];//초기값
			addRow(mygridG3,tCols);
