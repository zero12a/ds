...
require_once('menumngvService.php');
...
$log->info("MenumngvControl___________________________start");
...
}else if($objAuth->isAuth("MENUMNGV",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"MENUMNGV",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"MENUMNGV",$ctl,"N");
...
$REQ["G2-JSON"] = filterGridJson(
	array(
		"JSON"=>$REQ["G2-JSON"]
		,"COLORD"=>"MNU1_SEQ,FOLDER_YN,FOLDER_NM,PGMID,MNU_ICON,MNU_ORD,ADD_DT,MOD_DT"
		,"VALID"=>
			array(
...
$REQ["G3-JSON"] = filterGridJson(
	array(
		"JSON"=>$REQ["G3-JSON"]
		,"COLORD"=>"MNU2_SEQ,MNU1_SEQ,PGMID,MNU_ORD,ADD_DT,MOD_DT"
		,"VALID"=>
			array(
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new menumngvService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
	...
	case "G2_SAVE" :
		...
		break;
	case "G2_CHKSAVE" :
		echo $objService->goG2Chksave(); //MNU1, 선택저장
		break;
	case "G3_SEARCH" :
	...
	case "G3_SAVE" :
		...
		break;
	case "G3_CHKSAVE" :
		echo $objService->goG3Chksave(); //MNU2, 선택저장
		break;
	case "G4_SEARCH" :
...
$log->info("MenumngvControl___________________________end");
$log->close(); unset($log);
?>
