...
require_once('layout4fService.php');
...
$log->info("Layout4fControl___________________________start");
...
}else if($objAuth->isAuth("LAYOUT4F",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"LAYOUT4F",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"LAYOUT4F",$ctl,"N");
...
$REQ["G6-XML"] = getXml2Array($_POST["G6-XML"]);//	
//,  입력값 필터 
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new layout4fService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G2_SEARCHALL" :
	...
	case "G4_SAVE" :
		...
		break;
	case "G4_EXCEL" :
		echo $objService->goG4Excel(); //, 엑셀다운로드
		break;
	case "G4_CHKSAVE" :
		echo $objService->goG4Chksave(); //, 선택저장
		break;
	case "G5_SEARCH" :
		echo $objService->goG5Search(); //, 조회
		break;
	case "G5_SAVE" :
		echo $objService->goG5Save(); //, 저장
		break;
	case "G5_EXCEL" :
		echo $objService->goG5Excel(); //, 엑셀다운로드
		break;
	case "G5_CHKSAVE" :
		echo $objService->goG5Chksave(); //, 선택저장
		break;
	case "G6_SEARCH" :
		echo $objService->goG6Search(); //, 조회
		break;
	case "G6_SAVE" :
		echo $objService->goG6Save(); //, 저장
		break;
	case "G6_EXCEL" :
		echo $objService->goG6Excel(); //, 엑셀다운로드
		break;
	case "G6_CHKSAVE" :
		echo $objService->goG6Chksave(); //, 선택저장
		break;
	default:
...
$log->info("Layout4fControl___________________________end");
$log->close(); unset($log);
?>
