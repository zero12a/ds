...
require_once('layout3bService.php');
...
$log->info("Layout3bControl___________________________start");
...
}else if($objAuth->isAuth("LAYOUT3B",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"LAYOUT3B",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"LAYOUT3B",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new layout3bService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G2_SEARCHALL" :
...
$log->info("Layout3bControl___________________________end");
$log->close(); unset($log);
?>
