...
require_once('logloginService.php');
...
$log->info("LogloginControl___________________________start");
...
$REQ["G1-REMOTE_ADDR"] = reqPostString("G1-REMOTE_ADDR",20);//IP, RORW=RW, INHERIT=N, METHOD=POST
...
$REQ["G1-TO_DT"] = reqPostString("G1-TO_DT",10);//~, RORW=RW, INHERIT=N, METHOD=POST
$REQ["G1-TO_DT"] = getFilter($REQ["G1-TO_DT"],"REGEXMAT","/^[0-9]+$/");	

//G2, 목록 - RW속성 오브젝트만 필터 적용 ( RO속성은 제외 )
$REQ["G2-LOGIN_SEQ"] = reqPostNumber("G2-LOGIN_SEQ",10);//SEQ, RORW=RO, INHERIT=Y	
$REQ["G2-LOGIN_SEQ"] = getFilter($REQ["G2-LOGIN_SEQ"],"REGEXMAT","/^[0-9]+$/");	

//G3, 상세 - RW속성 오브젝트만 필터 적용 ( RO속성은 제외 )
$REQ["G3-LOGIN_SEQ"] = reqPostNumber("G3-LOGIN_SEQ",10);//SEQ, RORW=RW, INHERIT=N	
$REQ["G3-LOGIN_SEQ"] = getFilter($REQ["G3-LOGIN_SEQ"],"REGEXMAT","/^[0-9]+$/");	
$REQ["G3-AUTH_JSON"] = reqPostString("G3-AUTH_JSON",300);//AUTH, RORW=RW, INHERIT=N	
...
$REQ["G2-XML"] = filterGridXml(
	array(
		"XML"=>$REQ["G2-XML"]
		,"COLORD"=>"LOGIN_SEQ,USR_ID,SESSION_ID,SUCCESS_YN,USR_SEQ,SERVER_NAME,REMOTE_ADDR,USER_AGENT,ADD_DT"
		,"VALID"=>
			array(
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new logloginService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("LogloginControl___________________________end");
$log->close(); unset($log);
?>
