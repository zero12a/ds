...
require_once('cfghistoryService.php');
...
$log->info("CfghistoryControl___________________________start");
...
}else if($objAuth->isAuth("CFGHISTORY",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"CFGHISTORY",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"CFGHISTORY",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new cfghistoryService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("CfghistoryControl___________________________end");
$log->close(); unset($log);
?>
