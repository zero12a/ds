...
require_once('usermngwixlayoutService.php');
...
$log->info("UsermngwixlayoutControl___________________________start");
...
}else if($objAuth->isAuth("USERMNGWIXLAYOUT",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"USERMNGWIXLAYOUT",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"USERMNGWIXLAYOUT",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new usermngwixlayoutService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G2_USERDEF" :
...
$log->info("UsermngwixlayoutControl___________________________end");
$log->close(); unset($log);
?>
