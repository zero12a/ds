...
require_once('piinheritService.php');
...
$log->info("PiinheritControl___________________________start");
...
}else if($objAuth->isAuth("PIINHERIT",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"PIINHERIT",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"PIINHERIT",$ctl,"N");
...
$REQ["G1-CHILDGRPID"] = reqPostString("G1-CHILDGRPID",40);//CHILDGRPID, RORW=RW, INHERIT=N, METHOD=POST
$REQ["G1-CHILDGRPID"] = getFilter($REQ["G1-CHILDGRPID"],"REGEXMAT","/^[a-zA-Z]{1}[a-zA-Z0-9]*$/");	
...
$REQ["G2-INHERITSEQ"] = reqPostNumber("G2-INHERITSEQ",30);//INHERITSEQ, RORW=RO, INHERIT=Y	
$REQ["G2-INHERITSEQ"] = getFilter($REQ["G2-INHERITSEQ"],"REGEXMAT","/^[0-9]+$/");	
...
$REQ["G3-INHERITSEQ"] = reqPostNumber("G3-INHERITSEQ",30);//INHERITSEQ, RORW=RW, INHERIT=N	
$REQ["G3-INHERITSEQ"] = getFilter($REQ["G3-INHERITSEQ"],"REGEXMAT","/^[0-9]+$/");	
...
$REQ["G3-CHILDGRPID"] = reqPostString("G3-CHILDGRPID",40);//CHILDGRPID, RORW=RW, INHERIT=N	
$REQ["G3-CHILDGRPID"] = getFilter($REQ["G3-CHILDGRPID"],"REGEXMAT","/^[a-zA-Z]{1}[a-zA-Z0-9]*$/");	
//,  입력값 필터 
//,  입력값 필터 
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new piinheritService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("PiinheritControl___________________________end");
$log->close(); unset($log);
?>
