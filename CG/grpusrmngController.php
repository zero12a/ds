...
$REQ["G4-USR_ID"] = reqPostString("G4-USR_ID",10);//USR_ID, RORW=RW, INHERIT=N	
$REQ["G4-USR_ID"] = getFilter($REQ["G4-USR_ID"],"REGEXMAT","/^[a-zA-Z]{1}[a-zA-Z0-9]*$/");	
$REQ["G4-USR_NM"] = reqPostString("G4-USR_NM",10);//USR_NM, RORW=RW, INHERIT=N	
...
$REQ["G4-CHK"] = filterGridChk($REQ["G4-CHK"],"NUMBER",10,"REGEXMAT","/^[0-9]+$/");//USR_SEQ 입력값검증
	...
	//서비스 클래스 생성
$objService = new grpusrmngService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
