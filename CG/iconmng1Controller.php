...
require_once('iconmng1Service.php');
...
$log->info("Iconmng1Control___________________________start");
...
}else if($objAuth->isAuth("ICONMNG1",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"ICONMNG1",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"ICONMNG1",$ctl,"N");
...
$REQ["G3-CODESEARCH"] = reqPostString("G3-CODESEARCH",100);//CODESEARCH, RORW=RW, INHERIT=N	
...
$REQ["G2-XML"] = filterGridXml(
	array(
		"XML"=>$REQ["G2-XML"]
		,"COLORD"=>"ICONSEQ,IMGNM,IMGSVRNM,IMGSIZE,IMGHASH,IMGTYPE,IMGTYPE2,IMGTYPE3,IMGTYPE4,CODEMIRROR,TXTAREA,TXTVIEW,HTMLVIEW,SIGNPAD,CODESEARCH,ADDDT2,ADDDT"
		,"VALID"=>
			array(
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new iconmng1Service($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("Iconmng1Control___________________________end");
$log->close(); unset($log);
?>
