...
require_once('rdteamauthmngService.php');
...
$log->info("RdteamauthmngControl___________________________start");
...
}else if($objAuth->isAuth("RDTEAMAUTHMNG",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"RDTEAMAUTHMNG",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"RDTEAMAUTHMNG",$ctl,"N");
...
$REQ["G2-TEAM_SEQ"] = reqPostNumber("G2-TEAM_SEQ",100);//TEAM_SEQ, RORW=, INHERIT=Y	
$REQ["G2-TEAM_SEQ"] = getFilter($REQ["G2-TEAM_SEQ"],"REGEXMAT","/^[0-9]+$/");	
...
$REQ["G2-JSON"] = filterGridJson(
	array(
		"JSON"=>$REQ["G2-JSON"]
		,"COLORD"=>"TEAM_SEQ,TEAMCD,TEAMNM,USE_YN,ADD_DT,ADD_ID"
		,"VALID"=>
			array(
...
$REQ["G3-JSON"] = filterGridJson(
	array(
		"JSON"=>$REQ["G3-JSON"]
		,"COLORD"=>"CHK,TA_SEQ,TEAM_SEQ,PGMID,MNU_NM,AUTH_ID,AUTH_NM,ADD_DT,ADD_ID"
		,"VALID"=>
			array(
			"CHK"=>array("NUMBER",1)	
			,"TA_SEQ"=>array("NUMBER",20)	
			,"TEAM_SEQ"=>array("NUMBER",100)	
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new rdteamauthmngService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("RdteamauthmngControl___________________________end");
$log->close(); unset($log);
?>
