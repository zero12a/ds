...
require_once('jqxgridService.php');
...
$log->info("JqxgridControl___________________________start");
...
}else if($objAuth->isAuth("JQXGRID",$ctl)){
	$objAuth->LAUTH_SEQ = $objAuth->logUsrAuth($reqToken,$resToken,"JQXGRID",$ctl,"Y");
}else{
	$objAuth->logUsrAuth($reqToken,$resToken,"JQXGRID",$ctl,"N");
...
array_push($_RTIME,array("[TIME 40.REQ_VALID]",microtime(true)));
	//서비스 클래스 생성
$objService = new jqxgridService($REQ);
//컨트롤 명령별 분개처리
$log->info("ctl:" . $ctl);
switch ($ctl){
		case "G1_SEARCHALL" :
...
$log->info("JqxgridControl___________________________end");
$log->close(); unset($log);
?>
